...
import com.alat.interfaces.ALertElev
...
class ElevateAlert : AppCompatActivity(), AlertAdapter.ContactsAdapterListener {
    ...
    private fun getStudent() {
        ...
        val api: ALertElev = retrofit.create(ALertElev::class.java)
        val call: Call<ResponseBody> = api.getel(params)
    ...
    private fun parseLoginDatas(jsonresponse: String) {
        try {
            ...
            if (jsonObject.getString("status") == "true") {
                dialogue();
                promptPopUpView?.changeStatus(2, "ELEVATED SUCCESSFUL")
                mProgress?.dismiss()
                alert!!.dismiss()
